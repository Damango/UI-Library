...
const NavLink = (props) => {
    ...
    function handleDropDown(){
        if(dropDown){
           ...
           let containerTest = document.querySelector('.sub-link-container').offsetHeight
            setTimeout(() => { 
                console.log(containerTest)
                subLinksContainerRef.current.style.height = containerTest * props.data.subLinks.length + 'px'
            }, 0)
            setDropDown(true)
        }
    }
